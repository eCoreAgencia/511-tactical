...
import { isMobile, slugify, addToCart, changeQuantity } from "../utils";
...
class Product {
	constructor() {
		...
		$(".product__skus").on("click", ".sku-color", function(e) {
			...
			setTimeout(function() {
				$(".product__skus--color .colorSelect span").text(
					nameColor.replace(/.jpg|'|\)|/g, "").replace(/-/g, " ")
				);
			}, 1500);
		});
	...
	async renderSkuSelectors(product) {
		...
		const productJson = await getSearchProductByUrl(
	...
	createSkuSelect(items, sizes) {
		...
		return newSizes
			.map(size => {
				...
				if (skuI >= 0) {
					html = `<option rel="${items[skuI].available}" value="${
						items[skuI].sku
					}">${items[skuI].skuname}</option>`;
				}
		...
		return productFilters
			.map(product => {
				if (product.hasOwnProperty("ListaCores")) {
					return `<li><a style="background-image: url('/arquivos/${
				...
				return `<li><a title="" class="sku-color" href="#" id="product-color-${
					product.productId
				}" data-product-id="${product.productId}"></a></li>`;
			})
			.join("");
	...
	renderFormNotifyMe(product, sku) {
		...
		const html = `<div class="product__unavailable">
			...
			<form class="form" id="form-notifyme" action="/no-cache/AviseMe.aspx">
				<input type="hidden" name="notifymeIdSku" value="${sku ||
	...
	if ($("body").hasClass("product")) {
		...
		$(window).on("skuSelectorCreated", () => {
			...
			if ($(".product__skus li")[0]) {
				...
				const color = $(".product__skus li:first-child a")
					.attr("title")
					.replace(/-/g, " ");
			...
			$("select").each(function() {
				...
				$listItems.click(function(e) {
					...
					if ($(this).attr("available") == "false") {
						window.Product.renderFormNotifyMe(
		...
		$(".product__skus").on("click", ".select-options li", function() {
			console.log(this);
		});
	}
});
