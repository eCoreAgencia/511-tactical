...
$(document).ready(() => {
	...
	vtexjs.checkout.getOrderForm().done(async orderForm => {
		...
		if (isLogged) {
			let onMyAccount =
				sessionStorage.getItem("onMyAccount") ||
			...
			const welcomeMessage = userName
				? `Olá, ${userName}`
				: `Olá, ${userEmail}`;
			if (welcomeMessage.length >= 15)
				$(".logged-status").text(`${welcomeMessage.substr(0, 14)}...`);
			...
			if (R.isEmpty(user) || R.isNil(user)) {
				...
				if (user.approved == true) {
					...
					const notify =
						sessionStorage.getItem("userNotify") || false;
					if (!notify) {
						sessionStorage.setItem("userNotify", true);

						$(".modal-notify").addClass("is-active");
					}
				}
			}
		}
		...
		if (orderForm.userType == "callCenterOperator") {
			...
			if (
				!R.isEmpty(orderForm.clientProfileData) ||
				!R.isNil(orderForm.clientProfileData)
			) {
	...
	$(".buscarCnpj").on("click", function(e) {
		...
		const loadingImg =
			'<img src="https://casaeficiente2020.pt/Assets/Icons/loader.gif" style="position: absolute;top: 190px;background: #fff;width: 82%;padding: 0 170px;">';
		...
		$.ajax(ajaxValidCNPJSettings(cnpj))
			.done(function(response) {
				...
				if (Status) {
					const address = `${Enderecos[0].Logradouro}, ${
						Enderecos[0].Numero
					} ${Enderecos[0].Complemento}, ${Enderecos[0].Bairro} - ${
						Enderecos[0].Cidade
					} - ${Enderecos[0].Estado} - CEP ${Enderecos[0].CEP}`;
					...
					resultCnpj.append(
						`<div class='not-regitered--red'><p>${Mensagem}</p></div>`
					);
					setTimeout(function() {
	...
	$(".modal-notify .modal-close").on("click", function() {
		$(".modal-notify").removeClass("is-active");
	});
	...
	$(".confirmcad").on("click", async function(e) {
		...
		if (R.isEmpty(userCad) || R.isNil(userCad)) {
			...
			$.ajax({
				async: false,
				url:
					"//api.vtexcrm.com.br/tacticalb2b/dataentities/CL/documents/",
				type: "POST",
				contentType: "application/json",
				headers: {
				...
				success: function(data) {
					$("#createcliente .modalLogin__middle").html(
						'<div class="cadastroSucesso"><h3>Cadastrado com sucesso.</h3><p>Aguarde a liberação do seu perfil pela 5.11 para iniciar suas compras. Você será notificado por e-mail.</p></div>'
					);
					...
					if (!R.isNil(fileValue)) {
						...
						var settings = {
							...
							headers: {
								...
								"postman-token":
									"a7509754-c629-cba8-2ed6-195ce8307188"
							},
							processData: false,
							contentType: false,
					...
					$.ajax({
						async: false,
						url:
							"//api.vtexcrm.com.br/tacticalb2b/dataentities/AD/documents/",
						type: "PATCH",
						contentType: "application/json",
						headers: {
							Accept: "application/vnd.vtex.masterdata.v10+json",
